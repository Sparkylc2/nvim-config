		...
		config = function()
			...
			local function update_repo()
				if vim.fn.isdirectory(cache_dir .. "/.git") == 1 then
					...
					os.execute(
						("git clone --depth 1 %s %s"):format(
							vim.fn.shellescape(repo_url),
							vim.fn.shellescape(cache_dir)
						)
					)
				end
				return vim.fn.isdirectory(cache_dir .. "/template") == 1
			end
			...
			local function copy_template_here()
				...
				os.execute(
					("cp -R %s/. %s"):format(vim.fn.shellescape(cache_dir .. "/template"), vim.fn.shellescape(dest))
				)
