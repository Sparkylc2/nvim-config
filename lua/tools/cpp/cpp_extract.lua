...
local function has_template_ancestor(node)
	...
	while current do
		if current:type() == "template_declaration" then
			return true
		end
...
local function build_definition_prefix(bufnr, function_node, scope_prefix)
	...
	if not body_node or not declarator then
		return nil, "Unsupported function declarator"
	end

	local name_node = resolve_function_name_node(declarator)
	if not name_node then
		return nil, "Unsupported function name"
	end
...
local function ensure_cpp_include(cpp_path, include_line)
	...
	for _, line in ipairs(lines) do
		if trim(line) == include_line then
			return lines, false
		end
	end
...
local function append_missing_definitions(cpp_path, include_line, definition_entries, namespace_parts)
	...
	if appended > 0 then
		...
		if insert_at then
			...
			if previous_line and trim(previous_line) ~= "" then
				table.insert(chunk, "")
			end
			vim.list_extend(chunk, payload)
			if #chunk > 0 and trim(chunk[#chunk]) ~= "" then
				table.insert(chunk, "")
			end
...
local function build_definition_entry(bufnr, function_node, declaration_builder, scope_prefix, full_scope_prefix)
	...
	if not declaration or not definition then
		return nil, err
	end
	...
	local full_signature =
		normalize_ws((full_definition or definition):match("^(.-)%s*%b{}") or (full_definition or definition))
...
local function extract_class_functions(functions, class_node, bufnr, path)
	...
	for _, function_node in ipairs(functions) do
		local entry, err =
			build_definition_entry(bufnr, function_node, build_declaration, class_scope_prefix, full_scope_prefix)
		if entry then
...
local function extract_namespace_function(function_node, bufnr, path)
	...
	local entry, err =
		build_definition_entry(bufnr, function_node, build_free_function_declaration, "", full_scope_prefix)
	if not entry then
