...
local function find_enclosing_class(node, row, col)
	...
	local function walk(current)
		if not node_contains(current, row, col) then
			return
		end
...
local function get_class_name(class_node, bufnr)
	...
	if name_node then
		return trim(get_node_text(name_node, bufnr))
	end
...
local function contains_type(node, wanted)
	...
	for child in iter_named_children(node) do
		if contains_type(child, wanted) then
			return true
		end
	end
	return false
...
local function find_name_node(node)
	...
	if kind == "field_identifier" or kind == "identifier" then
		return node
	end
...
local function is_builtin_value_type(type_text)
	...
	if VALUE_PRIMITIVES[no_cv] then
		return true
	end
	if no_cv:find("[<>&*:%[%]%(%){}]", 1) then
		return false
	end

	local saw_primitive = false
	for token in no_cv:gmatch("[%a_][%w_]*") do
		if not VALUE_PRIMITIVES[token] then
			return false
		end
...
local function collect_members(bufnr, class_node)
	...
	if not body then
		return nil, "Cursor is not inside a C++ class definition"
	end

	if
		contains_type(body, "preproc_if")
...
local function constructor_param_types(bufnr, function_node)
	local declarator = get_field_child(function_node, "declarator")
		or find_first_named_child(function_node, "function_declarator")
...
local function is_constructor_node(bufnr, node, class_name)
	local kind = node:type()
	if kind ~= "declaration" and kind ~= "function_definition" then
		return false
	end
...
local function infer_indents(bufnr, body)
	...
	local fallback_child = class_indent
		.. string.rep(" ", vim.bo[bufnr].shiftwidth > 0 and vim.bo[bufnr].shiftwidth or 4)
...
function M.generate()
	...
	local existing_status =
		existing_constructor_status(bufnr, class_node, class_name, generated_param_types, member_names)
	...
	local _, _, body_end_row = body:range()
	local lines = build_constructor_lines(
		class_name,
		members,
		class_indent,
		child_indent,
		needs_public,
		insert_row < body_end_row
	)
